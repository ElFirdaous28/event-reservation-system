...
function EventMeta({ icon, children }: { icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className='text-muted flex items-center text-sm'>
      <span className='text-muted mr-2 h-4 w-4'>{icon}</span>
      {children}
    </div>
  );
}
...
export function EventCard({ event }: { event: Event }) {
  ...
  return (
    <div className='bg-surface border-border overflow-hidden rounded-lg border shadow-sm transition-shadow hover:shadow-md'>
      <div className='p-6'>
        <h2 className='text-foreground mb-2 text-xl font-semibold'>{event.title}</h2>
        ...
        {event._id ? (
          ...
          <button
            type='button'
            className='bg-primary/60 block w-full cursor-not-allowed rounded px-4 py-2 text-center font-medium text-white'
            disabled
          >
            View Details
          </button>
